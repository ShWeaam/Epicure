export const DESKTOP_WIDTH = "81.25vw"
export const MOBILE_WIDTH = "89.333vw"
export const DESKTOP_INNER_WIDTH = 0.9
export const MOBILE_LEFT_MARGIN = "7.462vw"
export const DESKTOP_LEFT_MARGIN = '9.375vw'
export const MOBILE_TO_DESKTOP_THRESHOLD = 768
export const ITEM_CARD_BG_COLOR = "rgb(249,244,234)"
export const FONT = 'Helvetica Neue, HelveticaNeue, Helvetica Neue Thin, HelveticaNeue Thin,HelveticaNeueThin, Helvetica Neue Medium, HelveticaNeue Medium, HelveticaNeueMedium'
export const FONT_COLOR = '#000000'
export const BG_COLOR = '#FFFFFF'
export const FONT_WEIGHT = 'lighter'
export const LETTER_SPACING = "1.92px"


export const DESKTOP_TEXT_SIZE = {
    h1: '32px',
    h2: '30px',
    h3: '40px',
    body: '24px',
    button: '16px',
    input: '16px',
    label: '14px',
    logo: '27px',
    navLink: '18px',
    footerLinks: '18px',
    foodCategory: '24px',
    chefOfTheWeekText: '24px',
    iconMeaning:'30px',
    searchBar:'24px',
    aboutUs:'20px',
};

export const MOBILE_TEXT_SIZE = {
    h1: '24px',
    h2: '16px',
    h3: '18px',
    body: '16px',
    button: '16px',
    input: '16px',
    label: '14px',
    navLink: '18px',
    footerLinks: '18px',
    foodCategory: '18px',
    chefOfTheWeekText: '18px',
    iconMeaning:'18px',
    searchBar:'17px',
    aboutUs:'18px',
};